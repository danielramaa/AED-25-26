...
public:  
  ...
  int diameter() {
    ...
    for (int i=1; i<=n; i++) {
      ...
      if (i == 1 && count != n) return -1;
      if (current_max > max_diam) max_diam = current_max;
    }
    return max_diam;
  }

  void dfsTarjan(int u, int &timer, int &scc_count, std::vector<int> &disc, std::vector<int> &low, std::vector<bool> &onStack, std::stack<int> &st) {
    disc[u] = low[u] = ++timer;
    st.push(u);
    onStack[u] = true;
    ...
    for (auto e : nodes[u].adj) {
      int v = e.dest;
      if (disc[v] == -1) {
        dfsTarjan(v, timer, scc_count, disc, low, onStack, st);
  ...
  int countSCCs() {
    ...
    for (int i = 1; i <= n; i++) {
      if (disc[i] == -1) {
        dfsTarjan(i, timer, scc_count, disc, low, onStack, st);
      }
    }
    return scc_count;
  }
